...
}};this.update=function(){var f=this.rte.selection.getNode();if(this.rte.dom.selfOrParentLink(f)){this.domElem.removeClass("disabled").addClass("active")}else{if(this.rte.dom.selectionHas(function(g){return g.nodeName=="A"&&g.href})){this.domElem.removeClass("disabled").addClass("active")}else{this.domElem.addClass("disabled").removeClass("active")}}}}})(jQuery);(function(a){elRTE.prototype.ui.prototype.buttons.youtube=function(f,c){this.constructor.prototype.constructor.call(this,f,c);this.youtube_url=a('<input type="text" />px').attr("name","youtube_url").attr("size","40");
this.youtube_w=a('<input type="text" />').attr("name","youtube_w").attr("size","12").val("560");this.youtube_h=a('<input type="text" />').attr("name","youtube_h").attr("size","12").val("315");var b=this;this.command=function(){var g,h;g={rtl:f.rtl,submit:function(j,k){j.stopPropagation();j.preventDefault();k.close();b.set(a("input[name=youtube_url]").val(),a("input[name=youtube_w]").val(),a("input[name=youtube_h]").val())},dialog:{width:460,title:this.rte.i18n("Insert YouTube video")}};this.rte.selection.saveIERange();
h=new elDialogForm(g);h.append([this.rte.i18n("Youtube URL"),this.youtube_url],null,true);h.append([this.rte.i18n("Width"),a("<span />").append(this.youtube_w).append(" px")],null,true);h.append([this.rte.i18n("Height"),a("<span />").append(this.youtube_h).append(" px")],null,true);h.open();this.rte.ui.update(true)};this.update=function(){this.domElem.removeClass("disabled active")};this.set=function(o,g,p){var q=function(t,h){var u=null;if(t.indexOf("?")!=-1){var x=t.split("?")[1].split("&"),z=[];
for(var w in x){var v=x[w].split("=");if(v.length>0){z[v[0]]=v[1]}}u=z;if(typeof h!="undefined"){if(typeof z[h]!="undefined"){u=z[h]}}}return u};var k='<iframe width="'+g+'" height="'+p+'" src="http://www.youtube.com/embed/'+q(o,"v")+'?wmode=transparent" frameborder="0" allowfullscreen></iframe>';var r="youtube-"+Math.random().toString().substring(2);this.rte.filter.scripts[r]=k;var j='<img id="'+r+'" src="'+this.rte.filter.url+'pixel.gif" class="elrte-protected elrte-iframe" style="width:'+g+"; height:"+p+'">';
this.rte.history.add();this.rte.selection.insertHtml(j)}}})(jQuery);elRTE.prototype.options.buttons.youtube="Insert Youtube video";