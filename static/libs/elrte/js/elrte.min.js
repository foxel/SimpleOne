...
}}}})(jQuery);(function(b){var a=function(f){var j=(f.match(/youtu\.be\/([^&\?]+)/)||f.match(/youtube.com\/watch\?(?:[^&]+&)*v=([^&\?]+)/)||[])[1];if(j){return"//www.youtube.com/embed/"+j+"?wmode=transparent"}var h=(f.match(/vimeo\.com\/([\w\-]+)/)||[])[1];if(h){return"//player.vimeo.com/video/"+h+"?wmode=transparent"}var c=(f.match(/rutube\.ru\/video\/([\w\-]+)/)||[])[1];if(c){return"//rutube.ru/play/embed/"+c}var g=(f.match(/instagram\.com\/p\/([\w\-]+)/)||[])[1];if(g){return"//instagram.com/p/"+g+"/embed/"
